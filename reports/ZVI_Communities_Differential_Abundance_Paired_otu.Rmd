---
title: "ZVI Communities Differential Abundance - Paired OTUs"
...
```
```{r}
# "Sample_ID" and "Sample_Code" are getting mixed up because that's how the table headings are in count_df.
# How can I group the individual table headings in count_df by Sample_Code? 
#tiff('CE_RB_Rel_Abund.tiff', units="in", width=8, height=6, res=500)
pdf("paired_otus.pdf",width=8,height=40)
ggplot(count_df) + 
    geom_boxplot(aes(color=type,y=Count,x=type)) +  
    facet_wrap(~genus, ncol = 2) +
    theme_bw() + labs(y = "CSS Normalized Abundance") +
    theme(legend.position = "bottom") +  geom_point(aes(color=Collection_Date,x=type,y=Count))
dev.off()
```